...
/* ─── formatDate ─── */
const formatDate = (dateStr: string, isAr: boolean) => {
  if (!dateStr) return "";
  try {
...
export default function NewsCarousel({ News }: { News: any[] }) {
  ...
  return (
    <div className="home-news-carousel">
      ...
      {/* SVG clip-path */}
      <svg className="carousel-clip-svg" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
        <defs>
      ...
      <div className="home-news-carousel-wrapper">
        <div className="home-news-grid">
          {News?.slice(0, 9).map((news, index) => (
            <div key={news.id ?? index} className="carousel-item">
              <div className="carousel-item-box">
                <div
                  className={`carousel-item-layout ${isAr ? "layout-rtl font-ar" : "layout-ltr font-en"}`}
                >
                  {/* عمود الصورة + الزرار */}
